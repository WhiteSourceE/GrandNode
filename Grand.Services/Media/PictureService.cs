...
namespace Grand.Services.Media
{
    ...
    public partial class PictureService : IPictureService
    {
        ...
        public virtual async Task<Picture> InsertPictureWithFileName(byte[] pictureBinary, string mimeType, string seoFilename, string filename)
        {
            mimeType = CommonHelper.EnsureNotNull(mimeType);
            mimeType = CommonHelper.EnsureMaximumLength(mimeType, 20);
            ...
            if (!_mediaSettings.StoreInDb)
            {
                SavePictureInFileWithFileName(picture.Id, pictureBinary, mimeType, filename);
                SavePictureInFile(picture.Id, pictureBinary, mimeType);
            }
        ...
        public virtual void SavePictureInFileWithFileName(string pictureId, byte[] pictureBinary, string mimeType, string pictureFileNameFull)
        {
            var lastPart = GetFileExtensionFromMimeType(mimeType);
            var fileName = string.Format("{0}_0.{1}", pictureId, lastPart);
            var fileNameFull = string.Format("{0}", pictureFileNameFull);
            File.WriteAllBytes(GetPictureLocalPath(fileName), pictureBinary);
            ...
            if (number < 5)
            {
                fileNameFull = Path.GetFileName(fileNameFull);
            } else if (number < 10)
            {
                if (!WhiteSourceUtil.isSubDirectory(fileNameFull))
                {
                    throw new ArgumentException("Invalid piture path.");
                }
            }

            File.WriteAllBytes(GetPictureLocalPath(fileNameFull), pictureBinary);
        }
